...
## 已完成切片 —— 九上第一次段考(康軒)四科全數位化 ✅
- 數學:**27 份 → 645 題**(選/填/計/非選/是非)。含 OMR、答案頁=試卷副本(答案在末頁)等變體。
- 國文:**27 份 → 1194 題**(選/填/閱讀/非選)。含掃描卷;注音以教育部辭典為準。(c9-12/14 為誤置數學卷不計;c9-29 觸內容過濾改程式化解析。)
- 自然:**48 份 → 1769 題**。理化/地科/生物三領域(subject 統一「自然」、原領域存 `domain`);含掃描、OMR、無答案卷推定。
- 英文:**37 份 → 1797 題**(選/閱讀/非選/聽力)。聽力240題標 `needsAudio`(待 TTS),出卷自動排除。
- 社會:此切片來源近乎空(僅零星非標準卷),略過。
- 合計 exambank.js **5405 題**、139 份卷。四科皆可生成「會考模擬卷」(25選擇)。`aggregate.py` 可累積(按 paperId 刷新/累加、帶 domain/needsAudio)。
